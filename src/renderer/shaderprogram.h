...
namespace Renderer 
{
	...
	@details Осуществляет создание шейдерной программы и её использование 
	...
	*/
	...
	public:
		...
		@details Создаёт вертексный и фрагментный шейдер и линкует их 
				 в шейдерную программу с определённым ID. В случае неудачи
		...
		*/
		...
		@details Выполняет перемещение шейдерной программы в новый объект, 
				 при этом информация о шейдерной программе в предыдущем 
				 объекте затирается.
		*/
	...
	private:
		...
		@details Создаёт и компилирует шейдер (вертексный или фрагментный) 
				 из исходного кода на языке GLSL. В случае неудачной компиляции 
				 выведет сообщение об ошибке.
		...
		*/
		...
		/// Флаг, хранящий состояние компиляции шейдерной программы
		bool compiled; 
